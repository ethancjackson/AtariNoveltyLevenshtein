...
"""
PURE NOVELTY SEARCH USING L DISTANCE
"""

num_cores = 16
game_name = 'Asteroids-v0'
exp_name = 'Asteroids-PureNovelty'
...
def evolve_solution(game_name, action_space_size, pop_size=1000+1, archive_p=0.01, archive_dump=None, n_generations=1000,
    ...
    for gen in range(starting_generation, n_generations):
        ...
        # Then select half with highest game score (Not in main experiment) - NOPE NOT IN PURE NOVELTY
        # pop_trunc = sorted(pop_trunc, key=lambda x: x[1])[-T // 2:]

        # Use cross-validation to select elite
        validation_pop = [result[0] for result in sorted(pop_trunc, key=lambda x: x[1])[-5:]]
        print([(x.init_seed, ns) for x, _, _, ns in pop_trunc[-5:]])
...
evolve_solution(game_name, action_space_size, pop_size=100+1, archive_p=0.1, archive_dump=None,
                training_episodes=[0], T=20, n_generations=500,
